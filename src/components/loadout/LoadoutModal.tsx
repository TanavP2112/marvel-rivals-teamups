        ...
        {!hero.unlocked ? (
          ...
          <>
            <div className='flex items-start justify-between border-b border-line px-7 py-6'>
              <div>
                <h2 className='mb-1.5 text-2xl font-black uppercase tracking-tight'>
                  {hero.name}
                </h2>
            ...
            <div className='grid grid-cols-1 gap-0 px-7 pb-2 pt-6 md:grid-cols-[1fr_auto_1fr]'>
              <LoadoutPanel
                label={hero.loadouts?.[0]?.name ?? 'Loadout A'}
              ...
              <LoadoutPanel
                label={hero.loadouts?.[1]?.name ?? 'Loadout B'}
            ...
            <VoteSplitBar
              votes={votes.overall}
              labelA={hero.loadouts![0].name ?? 'Loadout A'}
              labelB={hero.loadouts![1].name ?? 'Loadout B'}
            />
            ...
            {profile && (
              <div className='flex items-center justify-center gap-2 border-t border-line px-7 py-3 text-[11.5px] text-neutral-400'>
                <span>
                  Voting as {PLATFORM_LABEL[profile.platform]} ·{' '}
                  {REGION_LABEL[profile.region]} · {RANK_LABEL[profile.rank]}
                </span>
                <button
